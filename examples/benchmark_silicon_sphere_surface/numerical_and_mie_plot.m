...
%% plotting
fig_sca_silicon_sphere_numerical_mie_sur = figure;
line_weight = 1.5;
...
hall = plot(lam_para * 1e9, p, 'ok', 'MarkerSize', marker_size, 'linewidth', line_weight_marker);
p = (abs1a) ./ sigma_geom;
ha1 = plot(lam_para * 1e9, p, 'o', 'MarkerSize', marker_size, 'color', '#D95319', 'linewidth', line_weight_marker);
p = (abs1b) ./ sigma_geom;
hb1 = plot(lam_para * 1e9, p, 'o', 'MarkerSize', marker_size, 'color', '#0072BD', 'linewidth', line_weight_marker);
p = (abs2a) ./ sigma_geom;
ha2 = plot(lam_para * 1e9, p, 'o', 'MarkerSize', marker_size, 'color', '#7E2F8E', 'linewidth', line_weight_marker);
p = (abs2b) ./ sigma_geom;
hb2 = plot(lam_para * 1e9, p, 'o', 'MarkerSize', marker_size, 'color', '#EDB120', 'linewidth', line_weight_marker);
...
hlegend = legend('Total', 'ED', 'MD', 'EQ', 'MQ', 'surface');
...
%% save the image
res = 600;
filename = strcat(['fig_sca_silicon_sphere_numerical_mie_sur']);
filename = [strrep(filename, '.png', '') '.png'];
print(fig_sca_silicon_sphere_numerical_mie_sur, '-dpng', ['-r' num2str(res)], filename);
